...
export const useSupabaseStories = () => {
        ...
        try {
            ...
            if (existingBacklog) {
                ...
                if (existingBacklog.archived_at) {
                    ...
                    debug.info("fixed_supabase_stories.ts", '✅ Backlog sprint unarchived');
                }
            ...
            debug.error("fixed_supabase_stories.ts", "❌ Error ensuring backlog sprint exists", { err  });
    ...
    const fixBacklogPosition = useCallback(async () => {
        ...
        try {
            ...
            if (error)
                throw error;
            debug.info("fixed_supabase_stories.ts", "✅ Backlog position fixed", { backlogPosition  });
        }
        catch (err) {
            debug.error("fixed_supabase_stories.ts", "❌ Error fixing backlog position", { err  });
        }
    }, [user]);
    ...
    const createBacklogSprint = useCallback(async () => {
        ...
        if (error) {
            if (error.code === '23505') {
                debug.info("fixed_supabase_stories.ts", '⚠️ Backlog sprint already exists (race condition)');
                return;
            }
            throw error;
        }
        debug.info("fixed_supabase_stories.ts", '✅ Backlog sprint created successfully');
    }, [user]);
    ...
    const loadData = useCallback(async (forceRefresh = false) => {
        ...
        try {
            ...
            debug.info("fixed_supabase_stories.ts", "✅ Data loaded successfully", { {
            ...
            debug.error("fixed_supabase_stories.ts", "❌ Error loading data", { err  });
    ...
    const forceRefresh = useCallback(() => {
        debug.info("fixed_supabase_stories.ts", '🔄 Force refreshing data...');
        loadData(true);
    }, [loadData]);
    ...
    const addStory = useCallback(async (sprintId: string, storyData: {
        ...
        try {
            ...
            if (error)
                throw error;
            debug.info("fixed_supabase_stories.ts", "✅ Story added successfully", { storyNumber  });
            await loadData(true); // Force refresh after adding
        }
        catch (err) {
            debug.error("fixed_supabase_stories.ts", "❌ Error adding story", { err  });
    ...
    useEffect(() => {
        ...
        const sprintsSubscription = supabase
            ...
            debug.info("fixed_supabase_stories.ts", '🔄 Sprints changed, refreshing...');
            loadData(true);
        })
            .subscribe();
        const storiesSubscription = supabase
            ...
            debug.info("fixed_supabase_stories.ts", '🔄 Stories changed, refreshing...');
            loadData(true);
        })
            .subscribe();
        return () => {
