...
export const PromptProvider: React.FC<{
    ...
    const parseOpenSprintPrompt = (rawPrompt: string) => {
        ...
        if (startIndex === -1 || endIndex === -1) {
            debug.warn('Story format delimiters not found in open sprint prompt, using default format');
            return {
    ...
    const loadPromptsIfNeeded = () => {
        if (!isLoaded && user) {
            debug.info('Loading AI prompts on-demand');
            ...
            setIsLoaded(true);
            debug.info('AI prompts loaded successfully');
        }
    };
    ...
    const getStoryGenerationPrompt = (provider: 'openai' | 'anthropic', includeGithubCodeReview: boolean = false): string => {
        ...
        loadPromptsIfNeeded();
        if (!isLoaded) {
            debug.warn('Prompts not loaded yet, using fallback');
        ...
        switch (provider) {
            ...
            default:
                debug.warn('Unknown provider', { provider, message: 'using ChatGPT prompt as fallback' });
                return prompts.chatgpt;
        }
    };
    ...
    const getOpenSprintStoryFormat = (): string => {
        ...
        loadPromptsIfNeeded();
        if (!isLoaded) {
            debug.warn('Prompts not loaded yet, using fallback story format');
    ...
    return (<PromptContext.Provider value={value}>
      {children}
