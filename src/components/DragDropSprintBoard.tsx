...
export const DragDropSprintBoard: React.FC<DragDropSprintBoardProps> = ({ sprints, operationLoading = {}, getSprintStats, onAddStory, onOpenSprint, onCloseSprint, onDeleteSprint, onToggleStory, onMoveStory, onMoveSprint, onEditStory, onCloseBoard }) => {
    ...
    return (<DragDropContext onDragEnd={handleDragEnd}>
      <main className="p-6 max-w-none mx-auto">
        ...
        */}
        {allNonBacklogSprints.length > 0 && (<Droppable droppableId="sprints" type="sprint" direction="horizontal">
            {(provided, snapshot) => (<div ref={provided.innerRef} {...provided.droppableProps} className={`mb-5 transition-colors ${snapshot.isDraggingOver ? 'bg-devsuite-primary-subtle/30 rounded-lg p-4' : ''}`}>
                {/*
                ...
                */}
                <div className="grid grid-cols-2 gap-5">
                  {allNonBacklogSprints.map((sprint, index) => {
                    ...
                    return isDraggableSprint ? (<Draggable key={sprint.id} draggableId={sprint.id} index={index - 1}>
                      {(provided, snapshot) => (<div ref={provided.innerRef} {...provided.draggableProps} className={`transition-all ${snapshot.isDragging ? 'transform rotate-1 scale-105 z-50' : ''}`}>
                          <DroppableSprintCard id={sprint.id} title={sprint.title} icon={sprint.icon} stories={sprint.stories} stats={getSprintStats(sprint.id)} isDraggable={isDraggableSprint} operationLoading={operationLoading} dragHandleProps={provided.dragHandleProps} onAddStory={() => onAddStory(sprint.id)} onOpenSprint={() => onOpenSprint(sprint.id)} onCloseSprint={(type) => onCloseSprint(sprint.id, type)} onDeleteSprint={sprint.id === 'priority' ? undefined : () => onDeleteSprint(sprint.id)} onToggleStory={onToggleStory} onEditStory={onEditStory}/>
                        </div>)}
                      </Draggable>) : (
                    // Priority Sprint - not draggable and not deletable
                    <DroppableSprintCard key={sprint.id} id={sprint.id} title={sprint.title} icon={sprint.icon} stories={sprint.stories} stats={getSprintStats(sprint.id)} isDraggable={false} operationLoading={operationLoading} onAddStory={() => onAddStory(sprint.id)} onOpenSprint={() => onOpenSprint(sprint.id)} onCloseSprint={(type) => onCloseSprint(sprint.id, type)} onDeleteSprint={sprint.id === 'priority' ? undefined : () => onDeleteSprint(sprint.id)} onToggleStory={onToggleStory} onEditStory={onEditStory} onCloseBoard={sprint.id === 'priority' ? onCloseBoard : undefined}/>);
                })}
      ...
      */}
        ...
        */}
        {backlogSprint && (<div className="w-full">  {/* CRITICAL: Full width container for backlog */}
            <DroppableSprintCard id={backlogSprint.id} title={backlogSprint.title} icon={backlogSprint.icon} stories={backlogSprint.stories} stats={getSprintStats(backlogSprint.id)} isBacklog={true} // Explicitly set to true for backlog sprint
         operationLoading={operationLoading} isDraggable={false} // Explicitly prevent dragging
         onAddStory={() => onAddStory(backlogSprint.id)} onOpenSprint={() => onOpenSprint(backlogSprint.id)} onCloseSprint={(type) => onCloseSprint(backlogSprint.id, type)} onDeleteSprint={undefined} // Prevent deletion by not providing handler
         onToggleStory={onToggleStory} onEditStory={onEditStory}/>
          </div>)}
      </main>
    </DragDropContext>);
};
