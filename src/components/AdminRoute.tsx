...
export const AdminRoute: React.FC<AdminRouteProps> = ({ children }) => {
    ...
    if (!isAdmin) {
        ...
        <div className="bg-bg-primary border border-border-default rounded-xl p-8 max-w-md w-full text-center shadow-devsuite">
          ...
          <button onClick={() => window.history.back()} className="px-4 py-2 bg-devsuite-primary text-text-inverse rounded-lg hover:bg-devsuite-primary-hover transition-colors">
            Go Back
          </button>
        </div>
      </div>);
