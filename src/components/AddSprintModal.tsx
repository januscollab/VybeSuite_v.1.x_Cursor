...
export const AddSprintModal: React.FC<AddSprintModalProps> = ({ isOpen, onClose, onSubmit }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        onSubmit(formData.title.trim(), formData.icon.trim(), formData.description.trim(), false, // isBacklog - default to false
    ...
    return (<div className="fixed inset-0 bg-bg-overlay z-50 flex items-center justify-center p-5" onClick={(e) => {
      ...
      <div className="bg-bg-primary rounded-xl shadow-devsuite-modal border border-border-default w-full max-w-[500px]">
        ...
        <div className="px-6 py-4">
          {/* Error Message */}
          {error && (<div className="mb-4 p-3 bg-error-light border border-error rounded-lg">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input id="sprintTitle" type="text" value={formData.title} onChange={(e) => setFormData(prev => ({ ...prev, title: e.target.value }))} placeholder="Enter sprint title..." className="form-input" required/>
            ...
            <div>
              ...
              <textarea id="sprintDescription" value={formData.description} onChange={(e) => setFormData(prev => ({ ...prev, description: e.target.value }))} placeholder="Describe the goals and objectives of this sprint..." className="form-input resize-vertical min-h-[80px]" rows={3}/>
              <div className="text-xs text-text-tertiary mt-1">
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-6 gap-2">
                    {commonIcons.map((icon) => (<button key={icon} type="button" onClick={() => setFormData(prev => ({ ...prev, icon }))} className={`w-12 h-12 rounded-lg border-2 flex items-center justify-center text-xl transition-all hover:border-devsuite-primary hover:bg-devsuite-primary/5 ${formData.icon === icon
