...
export const OpenSprintModal: React.FC<OpenSprintModalProps> = ({ isOpen, sprint, onClose }) => {
    ...
    const handleCopyToClipboard = async () => {
        try {
            ...
            debug.error("OpenSprintModal", "Failed to copy to clipboard", { err  });
    ...
    return (<div className="fixed inset-0 bg-bg-overlay z-50 flex items-center justify-center p-5" onClick={(e) => {
      ...
      <div className="bg-bg-primary rounded-xl shadow-devsuite-modal border border-border-default w-full max-w-4xl max-h-[95vh] overflow-hidden">
        ...
        <div className="px-6 py-4 overflow-y-auto max-h-[calc(95vh-200px)]">
          {openStories.length === 0 ? (<div className="text-center py-12">
              ...
              <div>
                ...
                <textarea id="sprintPrompt" value={prompt} onChange={(e) => setPrompt(e.target.value)} className="w-full h-96 px-4 py-3 border border-border-default rounded-lg bg-bg-primary text-text-primary font-mono text-sm resize-vertical focus:outline-none focus:border-devsuite-primary focus:ring-2 focus:ring-devsuite-primary/20 transition-all" placeholder="Generated prompt will appear here..."/>
              ...
              <div className="bg-bg-muted rounded-lg p-4">
                ...
                <div className="space-y-2">
                  {openStories.map((story) => (<div key={story.id} className="flex items-start gap-3 p-2 bg-bg-primary rounded border border-border-subtle">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {story.description && (<p className="text-xs text-text-tertiary mt-1 line-clamp-2">{story.description}</p>)}
                        {story.tags.length > 0 && (<div className="flex flex-wrap gap-1 mt-2">
                            {story.tags.map((tag) => (<span key={tag} className="inline-block px-2 py-0.5 bg-devsuite-primary-subtle text-devsuite-primary text-xs rounded-full">
                                {tag}
                              </span>))}
                          </div>)}
                      </div>
        ...
        <div className="px-6 py-4 border-t border-border-default flex justify-between items-center">
          <div className="text-sm text-text-tertiary">
            {openStories.length > 0 && (<>Ready to copy prompt for {openStories.length} open {openStories.length === 1 ? 'story' : 'stories'}</>)}
          </div>
          
          <div className="flex items-center gap-3">
            <button type="button" onClick={onClose} className="flex items-center gap-2 px-4 py-2 bg-transparent text-text-secondary text-sm font-medium cursor-pointer border border-border-default rounded-lg transition-all hover:bg-bg-muted hover:text-text-primary">
              Cancel
            </button>
            
            {openStories.length > 0 && (<button onClick={handleCopyToClipboard} className="flex items-center gap-2 px-4 py-2 bg-devsuite-primary text-text-inverse text-sm font-medium cursor-pointer border border-devsuite-primary rounded-lg transition-all hover:bg-devsuite-primary-hover">
                {copied ? (<>
