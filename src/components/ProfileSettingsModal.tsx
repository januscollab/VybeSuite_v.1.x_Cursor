    ...
    return (<div className="fixed inset-0 bg-bg-overlay z-50 flex items-center justify-center p-5" onClick={(e) => {
      ...
      <div className="bg-bg-primary rounded-xl shadow-devsuite-modal border border-border-default w-full max-w-[500px] max-h-[95vh] overflow-hidden">
        ...
        <div className="px-6 py-4 overflow-y-auto max-h-[calc(95vh-140px)]">
          {loading && !userProfile ? (<div className="flex items-center justify-center py-12">
              <PulsingDotsLoader size="lg" className="mr-3"/>
              <span className="text-text-secondary">Loading profile...</span>
            </div>) : error ? (<div className="bg-error-light border border-error rounded-lg p-4">
              ...
              {/* Success Message */}
              {saveSuccess && (<div className="bg-success-light border border-success rounded-lg p-3">
                  <div className="flex items-center gap-2">
              ...
              {/* Error Message */}
              {saveError && (<div className="bg-error-light border border-error rounded-lg p-3">
                  <div className="flex items-center gap-2">
                    <AlertCircle className="w-4 h-4 text-error-dark"/>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    <input id="firstName" type="text" value={formData.firstName} onChange={(e) => setFormData(prev => ({ ...prev, firstName: e.target.value }))} placeholder="John" className="w-full px-3 py-2.5 border border-border-default rounded-lg bg-bg-primary text-text-primary placeholder-text-placeholder focus:outline-none focus:border-devsuite-primary focus:ring-2 focus:ring-devsuite-primary/20 transition-all"/>
                  </div>
                  <div>
                    <label htmlFor="lastName" className="block text-sm font-medium text-text-primary mb-2">
                      Last Name
                    </label>
                    <input id="lastName" type="text" value={formData.lastName} onChange={(e) => setFormData(prev => ({ ...prev, lastName: e.target.value }))} placeholder="Smith" className="w-full px-3 py-2.5 border border-border-default rounded-lg bg-bg-primary text-text-primary placeholder-text-placeholder focus:outline-none focus:border-devsuite-primary focus:ring-2 focus:ring-devsuite-primary/20 transition-all"/>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input id="email" type="email" value={formData.email} readOnly className="w-full px-3 py-2.5 border border-border-default rounded-lg bg-bg-muted text-text-secondary cursor-not-allowed"/>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input id="storyPrefix" type="text" value={formData.storyNumberPrefix} onChange={(e) => setFormData(prev => ({ ...prev, storyNumberPrefix: e.target.value }))} placeholder="STORY" className="w-full px-3 py-2.5 border border-border-default rounded-lg bg-bg-primary text-text-primary placeholder-text-placeholder focus:outline-none focus:border-devsuite-primary focus:ring-2 focus:ring-devsuite-primary/20 transition-all"/>
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      <select id="homepage" value={formData.preferredHomepage} onChange={(e) => setFormData(prev => ({ ...prev, preferredHomepage: e.target.value }))} className="w-full px-3 py-2.5 pr-10 border border-border-default rounded-lg bg-bg-primary text-text-primary focus:outline-none focus:border-devsuite-primary focus:ring-2 focus:ring-devsuite-primary/20 transition-all appearance-none cursor-pointer">
              ...
              <div>
                ...
                {passwordError && (<div className="mb-4 p-3 bg-error-light border border-error rounded-lg">
                    <div className="flex items-center gap-2">
                      <AlertCircle className="w-4 h-4 text-error-dark"/>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="relative">
                      <input id="newPassword" type={showNewPassword ? 'text' : 'password'} value={formData.newPassword} onChange={(e) => {
                setFormData(prev => ({ ...prev, newPassword: e.target.value }));
                setPasswordError(null); // Clear error when user types
            }} placeholder="Enter new password..." className="w-full px-3 py-2.5 pr-12 border border-border-default rounded-lg bg-bg-primary text-text-primary placeholder-text-placeholder focus:outline-none focus:border-devsuite-primary focus:ring-2 focus:ring-devsuite-primary/20 transition-all"/>
                      <button type="button" onClick={() => setShowNewPassword(!showNewPassword)} className="absolute right-3 top-1/2 transform -translate-y-1/2 p-1 text-text-quaternary hover:text-text-secondary transition-colors">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input id="confirmPassword" type={showConfirmPassword ? 'text' : 'password'} value={formData.confirmPassword} onChange={(e) => {
                setFormData(prev => ({ ...prev, confirmPassword: e.target.value }));
                setPasswordError(null); // Clear error when user types
            }} placeholder="Confirm new password..." className="w-full px-3 py-2.5 pr-12 border border-border-default rounded-lg bg-bg-primary text-text-primary placeholder-text-placeholder focus:outline-none focus:border-devsuite-primary focus:ring-2 focus:ring-devsuite-primary/20 transition-all"/>
                      <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-3 top-1/2 transform -translate-y-1/2 p-1 text-text-quaternary hover:text-text-secondary transition-colors">
        ...
        <div className="px-6 py-3 border-t border-border-default flex justify-end gap-3">
          <button type="button" onClick={onClose} disabled={saving} className="px-4 py-2 text-sm font-medium text-text-secondary border border-border-default rounded-lg hover:bg-bg-muted hover:text-text-primary transition-all disabled:opacity-50">
            Cancel
          </button>
          <button onClick={handleSubmit} disabled={saving || !userProfile} className="flex items-center gap-2 px-4 py-2 bg-devsuite-primary text-text-inverse text-sm font-medium rounded-lg hover:bg-devsuite-primary-hover transition-all disabled:opacity-50 disabled:cursor-not-allowed">
            {saving ? (<>
