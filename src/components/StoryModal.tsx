...
export const StoryModal: React.FC<StoryModalProps> = ({ isOpen, sprintId, sprintTitle, aiSettings, story, onClose, onSubmit, onUpdate, onDelete }) => {
    ...
    const handleGenerateStory = async () => {
        ...
        try {
            ...
            setAiGenerationWarning(result.description.length < 100 ||
    ...
    const RobotRunner = ({ message = "Generating story..." }) => (<div className="robot-container">
    ...
    if (!isOpen)
        return null;
    return (<>
    <div className="fixed inset-0 bg-bg-overlay z-50 flex items-center justify-center p-5" onClick={(e) => {
      ...
      <div className="bg-bg-primary rounded-xl shadow-devsuite-modal border border-border-default w-full max-w-[580px] max-h-[95vh] overflow-hidden">
        ...
        <div className="px-6 py-4 overflow-y-auto max-h-[calc(95vh-140px)]">
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="mb-3">
              ...
              <input id="storyTitle" type="text" value={formData.title} onChange={(e) => setFormData(prev => ({ ...prev, title: e.target.value }))} placeholder="Enter story title..." className="form-input-compact" required/>
            </div>
            ...
            {!isEditMode && (<div className="mb-3">
                ...
                <div className="flex items-center gap-3 mb-2">
                  <div className="flex gap-2">
                    {AI_PROVIDERS.map((provider) => (<button key={provider.id} type="button" onClick={() => setSelectedProvider(provider.id)} disabled={!hasValidApiKey(provider.id)} className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md transition-all flex-shrink-0 min-w-[70px] ${selectedProvider === provider.id
                    ...
                    : hasValidApiKey(provider.id)
                        ...
                        {provider.name}
                        {!hasValidApiKey(provider.id) && (<AlertCircle className="w-3 h-3"/>)}
                      </button>))}
                  </div>
                  
                  {/* GitHub Code Review Checkbox - Only show for Claude */}
                  {selectedProvider === 'anthropic' && (<label className="flex items-center gap-1.5 px-2 py-1 text-xs text-text-secondary cursor-pointer">
                      <input type="checkbox" checked={includeGithubCodeReview} onChange={(e) => setIncludeGithubCodeReview(e.target.checked)} className="w-3 h-3 text-devsuite-primary border-border-strong rounded focus:ring-devsuite-primary focus:ring-1"/>
                      include github code review
                    </label>)}
                </div>
                
                <textarea id="storyPrompt" value={storyPrompt} onChange={(e) => setStoryPrompt(e.target.value)} placeholder="Describe what you want to build... (e.g., 'Create a user login form with email validation')" className="form-input-compact resize min-h-[80px] max-h-[300px]" style={{
                resize: 'both',
                minWidth: '100%',
                maxWidth: '100%'
            }}/>
                ...
                {/* Generation Error */}
                {generationError && (<div className="mt-1.5 p-2 bg-error-light border border-error rounded-md">
                ...
                {storyPrompt.trim() && (<div className="flex items-center gap-2 mt-1.5">
                    <button type="button" onClick={handleGenerateStory} disabled={isGenerating || !hasValidApiKey(selectedProvider)} className="btn-primary-action text-xs py-1.5 px-2.5">
                      <Sparkles className={`w-3.5 h-3.5 ${isGenerating ? 'animate-spin' : ''}`}/>
                      {isGenerating ? 'Generating...' : `Generate with ${getProviderName(selectedProvider)}`}
                    </button>
                    
                    <button type="button" onClick={handleCreateStory} disabled={isGenerating} className="btn-primary-action text-xs py-1.5 px-2.5">
                      <Plus className="w-3.5 h-3.5"/>
                      Create Story
                    </button>
                  </div>)}
                
                {!hasValidApiKey(selectedProvider) && (<div className="text-[11px] text-text-tertiary mt-1">
            ...
            <div className="mb-3">
              <label htmlFor="description" className="form-label-compact">
                Description
                {aiGenerationWarning && (<span className="ml-2 text-xs text-warning-dark font-normal">
                    ⚠️ Consider expanding this description
                  </span>)}
              </label>
              <textarea id="description" value={formData.description} onChange={(e) => setFormData(prev => ({ ...prev, description: e.target.value }))} placeholder="Detailed description of the story..." className={`form-input-compact resize-vertical min-h-[70px] ${aiGenerationWarning
            ...
            <div className="mb-3">
              ...
              <div className="border-2 border-border-default rounded-lg bg-bg-primary p-1.5 min-h-[40px] flex flex-wrap gap-1.5 items-start transition-all focus-within:border-devsuite-primary focus-within:shadow-[0_0_0_3px_rgba(252,128,25,0.1)]">
                {formData.tags.map((tag) => (<div key={tag} className="bg-devsuite-primary-subtle text-devsuite-primary px-2 py-1 rounded-2xl text-xs font-medium flex items-center gap-1">
                    {tag}
                    <button type="button" onClick={() => removeTag(tag)} className="bg-transparent border-none text-devsuite-primary cursor-pointer p-0 w-3.5 h-3.5 rounded-full flex items-center justify-center text-[10px] leading-none hover:bg-devsuite-primary hover:text-white">
                      ×
                    </button>
                  </div>))}
                <input type="text" value={tagInput} onChange={(e) => setTagInput(e.target.value)} onKeyDown={handleTagInput} placeholder="Add tags (press Enter to add)..." className="border-none outline-none bg-transparent flex-1 min-w-[100px] p-1.5 text-[13px] text-text-primary placeholder-text-placeholder"/>
              </div>
              <div className="text-[11px] text-text-tertiary mt-0.5">
            ...
            {!isEditMode && (<div className="mb-3">
                ...
                <div className="priority-toggle">
                  <button type="button" onClick={() => setPriority('low')} className={`priority-option ${priority === 'low' ? 'active' : ''}`}>
                    🟢 Low
                  </button>
                  <button type="button" onClick={() => setPriority('medium')} className={`priority-option ${priority === 'medium' ? 'active' : ''}`}>
                    🟡 Medium
                  </button>
                  <button type="button" onClick={() => setPriority('high')} className={`priority-option ${priority === 'high' ? 'active' : ''}`}>
                    🔴 High
                  </button>
                </div>
              </div>)}
          ...
          {/* Delete Confirmation */}
          {showDeleteConfirmation && (<div className="mt-4 p-4 bg-error-light border border-error rounded-lg">
              ...
              <div className="flex gap-2">
                <button onClick={handleDelete} className="btn-primary-action bg-error hover:bg-error-dark">
                  <Trash2 className="w-4 h-4"/>
                  Delete Story
                </button>
                <button onClick={() => setShowDeleteConfirmation(false)} className="btn-secondary-action">
                  Cancel
                </button>
              </div>
            </div>)}
        ...
        <div className="px-6 py-3 border-t border-border-default flex justify-between items-center">
          <div className="flex items-center gap-2">
            {isEditMode && onDelete && (<button onClick={() => setShowDeleteConfirmation(true)} className="flex items-center gap-1.5 px-3 py-2 text-error hover:bg-error-light text-[13px] font-medium cursor-pointer border border-error rounded-md transition-all">
                <Trash2 className="w-4 h-4"/>
                Delete
              </button>)}
          </div>
          ...
          <div className="flex items-center gap-3">
            <button type="button" onClick={onClose} className="btn-secondary-action">
              Cancel
            </button>
            <button onClick={handleSubmit} disabled={!formData.title.trim()} className="btn-primary-action">
              {isEditMode ? (<>
